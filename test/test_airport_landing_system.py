...
class TestAirportLandingSystem(unittest.TestCase):
    ...
    def test_landing_at_busy_airport(self):
        ...
        self.system.airports["EMA"].landing_time = self.current_time - elapse
    ...
    def test_landing_after_waiting(self):
        ...
        self.system.airports["MAN"].landing_time = self.current_time - timedelta(seconds=60)
        can_land, _ = self.system.request_landing('MAN')
        self.assertTrue(can_land)

        self.system.airports["MAN"].landing_time = self.current_time - timedelta(seconds=59)
        can_land, _ = self.system.request_landing('MAN')
        self.assertFalse(can_land)
